...
* 废话不说,正确配置如下:(~/.bashrc在每个用户下都存在,我只是在root用户配置了,还有/etc/profile 也配置了)
  ...
  如果你是直接登录的普通用户,也会读取~/.bashrc文件,但是我没在改用户下配置此文件,所以依然找不到命令
  
  后来发现:/etc/profile 是属于全局的配置，每个用户还有属于自己的配置就是 ~/.bash_profile --而这个文件会取执行~/.bashrc 这个文件，属于用户自己的 会覆盖掉 全局的,所以..就找不到命令了.(因为当前用户加载的是自己的环境变量。覆盖了系统的环境变量)
